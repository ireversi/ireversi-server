...
router.route('/')
  .post(async (req, res) => {
        ...
        // 斜め方向
          ...
          if (ownPieces[i].y < result.y) {
            ...
            if (ownPieces[i].x < result.x) {
              ...
              // 置いたコマの下から次のコマまで
              for (let j = result.y - 1, k = result.x - 1; j > ownPieces[i].y, k > ownPieces[i].x; j -= 1, k -= 1) {

                // コマを取得
                const targetPiece = await PieceModel.find({x: k, y: j}, propFilter);
              ...
              if (flg) {
                for (let j = result.y - 1, k = result.x - 1; j > ownPieces[i].y, k > ownPieces[i].x; j -= 1, k -= 1) {
                  await PieceModel.update({x: k, y: j}, {$set:{userId: result.userId}});
                }
              }
            }
            else if (ownPieces[i].x > result.x) {
    ...
    allPieces = await PieceModel.find({}, propFilter);
